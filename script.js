...
function clearCart() {
  localStorage.removeItem('vexloi_cart');
  updateCartCounter();
  renderCartDrawer();
}
...
function renderCartDrawer() {
  ...
  drawerItems.innerHTML = cart.map(item => {
    ...
    total += itemTotal;
    return `
      <div class="flex items-center justify-between p-2 bg-gray-50 rounded border border-gray-200 gap-2">
        ...
        <div class="text-right flex flex-col items-end justify-between">
          <button onclick="removeFromCart('${item.id}')" class="text-gray-400 hover:text-red-600 p-1" title="Remove item">
            <i data-lucide="trash-2" class="w-3.5 h-3.5"></i>
          </button>
          <p class="text-xs font-black text-brand-dark mt-1">$${itemTotal.toFixed(2)}</p>
        </div>
      </div>
    `;
  }).join('');
...
function proceedToCheckout() {
  ...
  if (!activeUser) {
    alert('Please log in to proceed with your wholesale order.');
...
function renderCatalog(brand = 'all', category = 'all', search = '') {
  ...
  // Filtrado por Marca usando ID numérico entero
  if (brand !== 'all') {
    const targetBrandId = parseInt(brand, 10);
    filtered = filtered.filter(p => p.brandId === targetBrandId);
  }

  // Filtrado por Categoría (soporta ID entero o slug)
  if (category !== 'all') {
    const targetCatId = parseInt(category, 10);
    filtered = filtered.filter(p => {
      if (!isNaN(targetCatId)) {
        return p.categoryId === targetCatId;
      }
